...
def filter_and_sort_restaurants():
    ...
    num_restaurants = int(input("Enter the number of restaurants: "))
    ...
    min_rating_input = float(input("Now enter the minimum restaurant rating (or leave blank): "))
    min_rating = float(min_rating_input) if min_rating_input else 4.0
    
    filtered_restaurants = [entry["name"] for entry in restaurants if entry["rating"] >= min_rating]
